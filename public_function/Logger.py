...
class Logger(object):
    ...
    def __init__(self, filename=file_name, file_path='.', level='info', interval=1, when='D', backup_count = 5,
                 fmt='%(asctime)s - %(filename)s[line:%(lineno)d] - %(levelname)s: %(message)s', show_terminal=False):
        """ 初始化日志类
